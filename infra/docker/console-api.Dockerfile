...
FROM node:22-alpine
WORKDIR /app
COPY --from=builder /app/apps/console-api/dist ./dist
COPY --from=builder /app/apps/console-api/node_modules ./node_modules
COPY --from=builder /app/apps/console-api/package.json ./
EXPOSE 3000
CMD ["node", "dist/main"]
