...
프로젝트의 주요 의사결정은 `decisions/` 폴더에 기록한다.
**RFC(논의·합의) → 승격 → ADR(확정·박제)** 하나의 파이프라인.

```
decisions/
├── README.md      # 의사결정 인덱스 + 프로세스 (이 절의 요약 + 폴더 안내)
├── ADR/           # 내려진 결정 (확정 후 본문 불변)
│   ├── README.md  # ADR 목록 표
│   └── ADR-NNN-제목.md
└── RFC/           # 결정에 이르는 토론 (살아 움직이는 문서)
    ├── README.md  # RFC 목록 표 + 상태 정의
    ├── TEMPLATE.md
    └── RFC-NNN-제목.md
```

- **RFC** = *결정에 이르는 토론*. 여러 선택지를 펼치고 합의를 모은다.
- **ADR** = *내려진 결정*. 합의 결과를 박제한다. 확정 후 본문 수정 금지.
- 작고 자명한 결정은 RFC 없이 **바로 ADR**. *대안이 다투거나 합의가 필요한* 결정만 RFC를 거친다.

### 언제 무엇을 쓰나

| 상황 | 도구 |
| --- | --- |
| 대안이 다툼 / 합의 필요 / 되돌리기 비싼데 확신 없음 / 미해결 질문 여럿 | RFC → (합의) → ADR |
| 대안이 자명, 합의 불필요, 되돌리기 비용 작음 | 바로 ADR |
| 일상 버그 수정·리팩터링·1~3줄 변경·단순 문서 갱신 | `.claude-log/` (RFC/ADR 불필요) |

ADR 작성 트리거: 기술/라이브러리 선정·변경, 아키텍처·프로토콜·인터페이스 결정,
기획 요구 해석에 따른 설계 방향 결정, **기존 결정을 번복하는 경우**.

### ADR 규칙

- 경로: `decisions/ADR/ADR-NNN-제목.md`, 번호 순차 부여(001~), 재사용·재정렬 금지(폐기돼도 결번 유지).
- 상태: `제안` → `확정` → `대체됨` / `폐기`.
- **확정된 ADR은 본문을 수정하지 않는다.** 결정이 바뀌면 새 ADR 작성 + 기존 ADR 상태를
  `대체됨`으로 변경(어느 ADR이 대체하는지 명시). 부분 대체도 상태 칸에 명시 가능.
- 새 ADR 작성 시 `decisions/ADR/README.md` 목록 표에 한 줄 추가.

### RFC 규칙

- 경로: `decisions/RFC/RFC-NNN-제목.md`, `TEMPLATE.md` 복사해 작성. 번호 순차 부여.
- 상태: `제안` → `논의 중` → `합의` → `확정으로 승격` / `폐기` / `보류`.
- *미해결 질문*이 RFC의 심장. 항목이 무거우면 후속 RFC로 분리(RFC 하나가 모든 걸 결정하려 들지 않음).
- *추천*을 명시해야 토론이 수렴한다. 선택지만 나열하지 않는다.
- 새 RFC 작성 시 `decisions/RFC/README.md` 목록 표에 한 줄 추가.

### RFC → ADR 승격 규칙

RFC가 `합의`에 도달하면 ADR로 승격한다.

1. **ADR 신규 작성** — RFC의 *추천 옵션*을 `결정`으로, *검토 선택지/근거*를 ADR 템플릿에
   압축 이식. 상세 토론은 RFC 링크로 위임.
2. ADR `관련 RFC` 칸에 원본 RFC 링크 명시.
3. RFC 상태를 `확정으로 승격`으로 변경, RFC 상단 `관련 ADR`에 새 ADR 링크.
4. **RFC 원문은 보존**(삭제 금지 — 논의 맥락은 ADR보다 풍부한 1차 사료).
5. RFC의 무거운 *미해결 질문*은 후속 RFC로 분리해 별도 승격.

승격 안 함: RFC `폐기`(채택 안 함, 단 "왜 안 했나"가 가치 있으면 ADR로 박제 가능) /
RFC `보류`(재검토 대기, 트리거 충족 시 `논의 중` 복귀).
...
- **일자**: YYYY-MM-DD
- **결정자**: 이름
- **관련 RFC**: (없음 / RFC-NNN 링크 — RFC에서 승격된 경우)

## 배경
왜 이 결정이 필요한지 (문제·제약·트리거)

## 검토 선택지
1. **선택지 A** → 장단점
2. **선택지 B** → 장단점   (기각안도 *기각 이유와 함께*)

## 결정
최종 선택 (한 문장으로 명확히)

## 근거
왜 이것을 선택했는지 (선택지 비교의 결론)

## 영향
이 결정으로 바뀌거나 추가 확인이 필요한 사항 (미확인 항목은 "미확인 사항"으로 명시)
```
