...
- 제이랩이 개발한 외부 SaaS를 회사가 고객으로 구독하는 구조
- "회사가 외부 도구를 사용하는 것"이지 "회사 업무용으로 개발한 것"이 아님
- 가능하면 사내 도입 전에 외부 공개(GitHub, 랜딩페이지) 선행

**권장 사항:**

- 고용 계약서의 직무발명/겸업 조항 확인
- 필요 시 회사와 서면 합의 (개인 프로젝트 인정 + 사용 허가)
- 초기 단계에서 변호사 상담 (IT/스타트업 전문)

> ⚠️ 본 섹션은 법률 자문이 아닌 참고 가이드이며, 실제 진행 시 법률 전문가 상담 필요.
